class PsychologistAvailabilitiesController < ApplicationController
  ...
  # GET /psychologist_profiles/:psychologist_profile_id/psychologist_availabilities
   def index
    ...
    # NEW DEBUGGING LINE: Log the timezone being used.
    Rails.logger.debug "Displaying availabilities for timezone: #{@psychologist_profile.timezone}"
    
    Rails.logger.debug "Loaded availabilities: #{@availabilities.map { |a| { id: a.id, day: a.day_of_week, start: a.start_time_in_zone, end: a.end_time_in_zone } }}"
  end
