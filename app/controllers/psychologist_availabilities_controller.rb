  ...
  def update_all
    ...
    ActiveRecord::Base.transaction do
      ...
      submitted_slots_data.each do |slot_data|
        ...
        dow       = slot_data[:day_of_week]

        # Skip empty rows
        next if start_str.blank? || end_str.blank?

        begin
          start_parsed = Time.strptime(start_str, "%H:%M")
          end_parsed   = Time.strptime(end_str, "%H:%M")
          ...
          errors << "Invalid time format (expected HH:MM)"
          next
        end
        ...
        if is_existing
          ...
          if availability
            availability.assign_attributes(
              day_of_week:      dow,
              start_time_of_day: start_str,
              end_time_of_day:   end_str,
              timezone:          tz
            )
    ...
    if errors.any?
      redirect_to psychologist_profile_psychologist_availabilities_path(@psychologist_profile),
                  alert: "Availability could not be saved: #{errors.join(', ')}"
    else
      redirect_to psychologist_profile_psychologist_availabilities_path(@psychologist_profile),
                  notice: "Availability successfully saved!"
    end
  end
...
def calendar_blocks
  tz = @psychologist_profile.timezone || params[:browser_timezone] || session[:browser_timezone] || cookies[:browser_timezone]  
  availability_zone = ActiveSupport::TimeZone[tz]
  ...
  availabilities.each do |a|
    zone = ActiveSupport::TimeZone[a.timezone || tz]

    (start_date.to_date..end_date.to_date).each do |date|
      next unless date.wday == a.day_of_week

      start_local = zone.local(date.year, date.month, date.day,
                              a.start_time_of_day.hour, a.start_time_of_day.min)
      end_local   = zone.local(date.year, date.month, date.day,
